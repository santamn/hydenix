  ...
  config = lib.mkIf cfg.enable {
    ...
    programs.fish = lib.mkIf cfg.fish.enable {
      ...
      interactiveShellInit = ''
        ...
        ''}
      '';
      # 以前は interactiveShellInit の中に `alias ...` をベタ書きしていたが、
      # home-manager のオプションで宣言する形に統一した。
      # 利用者が同じ名前を上書きでき、fish の abbr は shellAbbrs 側に分けられる。
      # あわせて Arch の AUR ヘルパー向けエイリアス（un/up/pl/pa/pc/po）を削除。
      # NixOS には $aurhelper が存在せず、実行しても動かなかった
      shellAliases = {
        c = "clear";
        l = "eza -lh --icons=auto";
        ls = "eza -1 --icons=auto";
      ...
      # abbr は入力後に展開される略語。alias と違い履歴に展開後が残る
      shellAbbrs = {
        ".." = "cd ..";
        "..." = "cd ../..";
        ".3" = "cd ../../..";
